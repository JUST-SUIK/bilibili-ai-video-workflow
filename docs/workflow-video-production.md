...
- **2026-07-02**：v2.1.1 红队审查全覆盖 + AND-join 并行汇合 + 诚实标注已知限制

---

## 九、远期改进储备（当前不做，规模上来后再启用）

| 改进项 | 暂缓原因 | 启用条件 |
|--------|---------|---------|
| **Git worktree 隔离** | HTML(frames/) 和 TTS(audio/) 写不同目录，无共享写入点，asyncio 并发已足够 | 出现"多个 Agent 同时改同一文件"的需求时 |
| **定时/cron 触发** | 一两周一个视频，人在电脑前手动启动即可 | 日产 ≥1 个视频，或需要夜间无人值守运行时 |
| **budget.py 独立模块** | 单视频几十万 token，一个计数器变量就够 | 单次运行 token 消耗 ≥ 500 万，或需要多级预算策略时 |
| **技能渐进式加载** | 当前 skills/ 规模小，一次性加载 token 开销可接受 | skills 目录 ≥ 20 个文件时 |
| **上下文显式注入机制** | Studio 引擎不支持，且对话历史传递在当前规模下尚可靠 | 单条流水线 ≥ 15 个 Agent 节点，上下文截断频繁出现时 |
| **定时封面图生成 Agent** | 需要图像生成 API（Midjourney/DALL-E）集成，目前手动做图可控 | 日产 ≥ 3 个视频，手动做图成为瓶颈时 |
| **已发布视频数据反馈闭环** | B站 API 数据获取 + 结构化分析复杂度高 | 累计发布 ≥ 10 个视频，需要数据驱动选题时 |
